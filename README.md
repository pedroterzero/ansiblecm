...
```bash
docker build --tag ansiblecm:10.3.0 .
```
...
```bash
docker run -it --rm -v <absolute path to playbook>:/tmp/playbook:Z jmal98/ansiblecm:10.3.0 <playbook arguments>
```
...
```bash
docker run -it --rm -v $PWD:/tmp/playbook:Z jmal98/ansiblecm:10.3.0 site.yml -i inventory/hosts
```
...
```bash
docker run -it --rm --entrypoint ansible jmal98/ansiblecm:10.3.0 web -m setup -i inventory
```
